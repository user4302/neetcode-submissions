class Solution {
    ...
    searchMatrix(matrix, target) {
        ...
        for (let row = 0; row < matrix.length; row++) {
            ...
            console.log('matrix[row][0]>=target',matrix[row][0]>=target)
            console.log('matrix[row][matrix[row].length-1]<=target',matrix[row][matrix[row].length-1]<=target)
            
            if(matrix[row][0]<=target && matrix[row][matrix[row].length-1]>=target){
                found = matrix[row].includes(target)
            }
        }
        return found
    }
