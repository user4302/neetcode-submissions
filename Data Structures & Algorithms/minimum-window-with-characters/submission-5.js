class Solution {
    ...
    minWindow(s, t) {
        ...
        for (let leaderPointer = 0; leaderPointer < s.length; leaderPointer++) {
            ...
            let currentLeaderPointerCharacterInS = s[leaderPointer];
            if (currentLeaderPointerCharacterInS in requiredCharacters) {
                // only if the currentLeaderPointerCharacterInS is in requiredCharacters
                // we create a KV pair in availableCharacters
                availableCharacters[currentLeaderPointerCharacterInS] =
                    (availableCharacters[currentLeaderPointerCharacterInS] || 0) + 1;

                // if the currentLeaderPointerCharacterInS value is equal to
                ...
                if (
                    availableCharacters[currentLeaderPointerCharacterInS] ===
                    requiredCharacters[currentLeaderPointerCharacterInS]
                ) {
                    validCharacterTypes++;
                }
            }
            ...
            while (validCharacterTypes === uniqueCharacterTypesCount) {
                ...
                // X, only save if the window is actually smaller than previous
                ...
                // followerPointer++;
                // X, If leftChar is a needed character
                // (i.e., it exists in requiredCharacters),
                // you must decrement its count from your active window
                let leftCharacterOfFollowerPointer = s[followerPointer];
                if (leftCharacterOfFollowerPointer in requiredCharacters) {
                    availableCharacters[leftCharacterOfFollowerPointer]--;

                    // If our window no longer has ENOUGH copies of this character,
                    // validCharacterTypes--
                    if (
                        availableCharacters[leftCharacterOfFollowerPointer] <
                        requiredCharacters[leftCharacterOfFollowerPointer]
                    ) {
                        validCharacterTypes--;
                    }
                }
        ...
        // here if shortestWindowLength === Infinity, the value did not change, so
        return shortestWindowLength === Infinity
            ? // no valid substring was found so return ""
              ""
